...
namespace SmartApp.Infrastructure.Repository
{
    public class ReservationRepository : IReservationRepository
    {
        ...
        public async Task<DiningTableWithTimeSlotModel> CheckInReservationAsync(DiningTableWithTimeSlotModel reservation)
        {
            var existingUser = await _dbContext.Users
                .FirstOrDefaultAsync(u => u.Email.ToLower() == reservation.UserEmailId);

            if (existingUser == null)
            {
                throw new Exception($"User with email {reservation.UserEmailId} not found.");
            }

            var timeSlotToUpdate = await _dbContext.TimeSlots
                .FirstOrDefaultAsync(t => t.Id == reservation.TimeSlotId);

            if (timeSlotToUpdate == null)
            {
                throw new Exception($"TimeSlot with Id {reservation.TimeSlotId} not found.");
            }
            ...
            var reservationToUpdate = await _dbContext.Reservations
                .FirstOrDefaultAsync(r => r.TimeSlotId == reservation.TimeSlotId);

            if (reservationToUpdate == null)
            {
                throw new Exception($"No reservation found for TimeSlotId {reservation.TimeSlotId}.");
            }
